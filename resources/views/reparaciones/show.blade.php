...
@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="{{ route('reparaciones.index') }}" style="color: #a855f7;">Reparaciones</a>
    </li>
...
<div class="d-flex align-items-center justify-content-between mb-4 btn-acciones">
    <div>
        ...
        <p class="text-muted mb-0" style="font-size: 13px;">
            Recibido el {{ $reparacion->fecha_recepcion ? $reparacion->fecha_recepcion->format('d/m/Y H:i') : 'Fecha no registrada' }} ·
    ...
    <div class="d-flex gap-2">
        ...
        <a href="/reparaciones/{{ $reparacion->id }}/edit" class="btn btn-primary px-4">
        ...
        <div class="card mb-3">
            <div class="card-body p-4">
                ...
                <div class="d-flex justify-content-between mb-2" style="font-size: 13px;">
                    ...
                    <span>{{ $reparacion->fecha_recepcion ? $reparacion->fecha_recepcion->format('d/m/Y') : 'Fecha no registrada' }}</span>
                </div>
                @if($reparacion->fecha_estimada)
                <div class="d-flex justify-content-between mb-2" style="font-size: 13px;">
                    <span class="text-muted">Fecha estimada</span>
                    <span>{{ $reparacion->fecha_estimada ? $reparacion->fecha_estimada->format('d/m/Y') : 'No definida' }}</span>
                </div>
                @endif
                @if($reparacion->fecha_entrega)
                <div class="d-flex justify-content-between mb-2" style="font-size: 13px;">
                    <span class="text-muted">Entregado</span>
                    <span style="color: #059669; font-weight: 600;">{{ $reparacion->fecha_entrega ? $reparacion->fecha_entrega->format('d/m/Y') : 'No entregado' }}</span>
                </div>
                @endif
                ...
                @if($reparacion->cliente->email ?? false)
                    <div style="font-size: 12px; color: #9ca3af;">{{ $reparacion->cliente->email }}</div>
                @endif

                <div class="mt-3 d-grid gap-2">
                    <!-- ⭐ CORREGIDO - URL directa -->
                    <a href="/reparaciones/{{ $reparacion->id }}/edit" class="btn btn-primary btn-sm">
                        <i class="fas fa-edit me-1"></i>Actualizar Estado
                    </a>
                    <!-- ⭐ CORREGIDO - URL directa -->
                    <a href="/clientes/{{ $reparacion->cliente_id }}" class="btn btn-outline-secondary btn-sm">
