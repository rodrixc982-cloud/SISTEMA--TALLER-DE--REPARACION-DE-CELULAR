...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($cats as $cat) {
            if (!Categoria::where('nombre', $cat)->exists()) {
        ...
        foreach ($marcas as $marca) {
            if (!Marca::where('nombre', $marca)->exists()) {
        ...
        foreach ($clientes as $c) {
            // Verificar por email si existe, o por DNI si no tiene email
            $exists = false;
            if ($c['email']) {
                $exists = Cliente::where('email', $c['email'])->exists();
            } else {
                $exists = Cliente::where('dni', $c['dni'])->exists();
            }
